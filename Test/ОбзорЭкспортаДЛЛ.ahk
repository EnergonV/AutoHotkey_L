...
global app           := { name: "Обозреватель Экспорта DLL", version: "0.6.2", release: "2018-07-23" }
...
if (WinVersion < 0x0501) {
    MsgBox, 0x1030, % "Требования", % "Как минимум, требуется операционная система Windows XP или Windows Server 2003"
    ExitApp
}
...
Menu, FileMenu, Add, % "&Открыть...`tCtrl+O",      MenuOpenFile
Menu, FileMenu, Add, % "Загрузить Общие Dll's",     MenuOpenCommonFiles
Menu, FileMenu, Add
Menu, FileMenu, Add, % "В&ыход`tAlt+F4",         MenuClose
Menu, MenuBar,  Add, % "&Файл",                :FileMenu

Menu, EditMenu, Add, % "Копировать",                  MenuCopy
Menu, EditMenu, Add, % "Выбрать Все`tCtrl+A",    MenuSelect
Menu, EditMenu, Add
Menu, EditMenu, Add, % "ИнфОФункц`tCtrl+F", MenuShowFInfo
Menu, EditMenu, Add, % "ИнфОМодуле`tCtrl+M",   MenuShowMInfo
Menu, MenuBar,  Add, % "&Редактировать",                :EditMenu

Menu, ViewMenu, Add, % "Развернуть",           MenuCollapse
Menu, ViewMenu, % (IsExpandView) ? "Check" : "UnCheck", % "Развернуть"
Menu, MenuBar,  Add, % "&Обзор",                :ViewMenu

Menu, HelpMenu, Add, % "&Программа`tF1",            MenuAbout
Menu, MenuBar,  Add, % "&Помощь",                :HelpMenu
...
Gui, Main: Add, ListView, xm ym w800 h450 vMyLV1 hWndhMyLV1 +LV0x14000, % "#|Имя Функции|Порядковый|Точка Входа (RVA)|Имя Модуля|Путь к Модулю"
...
Gui, Main: Add, ListView, x+7 ym w200 h450 gMyLV2 vMyLV2 hWndhMyLV2 +LV0x14000, % "ИмяМодуля"
GuiControl, % (IsExpandView) ? "Показать" : "Скрыть", MyLV2
LV_ModifyCol(1, 178)
...
EM_SETCUEBANNER(hMyEdit1, "Введите запрос")
...
ChildFunctionInfo(function, rva, ordinal, module, path)
{
    ...
    Gui, FInfo: Add, Text, xm ym w180 h25 0x200, % "Имя Функции:"
    Gui, FInfo: Add, Edit, x+1 yp w350 0x800, % function
    Gui, FInfo: Add, Text, xm y+4 w180 h25 0x200, % "Точка Входа (RVA):"
    Gui, FInfo: Add, Edit, x+1 yp w350 0x800, % rva
    Gui, FInfo: Add, Text, xm y+4 w180 h25 0x200, % "Порядковый:"
    Gui, FInfo: Add, Edit, x+1 yp w350 0x800, % ordinal "   (" Format("0x{:x}", ordinal) ")"
    Gui, FInfo: Add, Text, xm y+4 w180 h25 0x200, % "Имя Модуля:"
    Gui, FInfo: Add, Edit, x+1 yp w350 0x800, % module
    Gui, FInfo: Add, Text, xm y+4 w180 h25 0x200, % "Полный Путь:"
    Gui, FInfo: Add, Edit, x+1 yp w350 0x800, % path
    Gui, FInfo: Add, Text, xm y+4 w180 h25 0x200, % "РаздекорироватьИмяСимвола:"
    Gui, FInfo: Add, Edit, xm y+4 w534 r3 0x800, % UnDecorateSymbolName(function)
    Gui, FInfo: Show, AutoSize, % "ИнфОФункции"
    WinWaitClose
    return
...
ChildModuleInfo(ModulePath)
{
    ...
    Gui, MInfo: Add, Text, xm ym w180 h25 0x200, % "ИмяМодуля:"
    Gui, MInfo: Add, Edit, x+1 yp w350 0x800, % ModuleName
    Gui, MInfo: Add, Text, xm y+4 w180 h25 0x200, % "ИсходноеИмяФайла:"
    Gui, MInfo: Add, Edit, x+1 yp w350 0x800, % FileInfo.OriginalFilename
    Gui, MInfo: Add, Text, xm y+4 w180 h25 0x200, % "НазваниеКомпании:"
    Gui, MInfo: Add, Edit, x+1 yp w350 0x800, % FileInfo.CompanyName
    Gui, MInfo: Add, Text, xm y+4 w180 h25 0x200, % "ВерсияПродукта:"
    Gui, MInfo: Add, Edit, x+1 yp w350 0x800, % FileInfo.ProductVersion
    Gui, MInfo: Add, Text, xm y+4 w180 h25 0x200, % "ВерсияФайла:"
    Gui, MInfo: Add, Edit, x+1 yp w350 0x800, % FileInfo.FileVersion
    Gui, MInfo: Show, AutoSize, % "ИнфОМодуле"
    WinWaitClose
    return
...
MainContextMenu:
    ...
    if (A_GuiControl = "MyLV1") && (LV_GetNext() > 0) {
        Menu, ContextMenuLV1, Add, % "Копировать", ContextCopy
        Menu, ContextMenuLV1, Add, % "Выбрать Все`tCtrl+A", ContextSelect
        Menu, ContextMenuLV1, Add, % "ИнфОФункции`tCtrl+F", ContextShowFInfo
        Menu, ContextMenuLV1, Add, % "ИнфОМодуле`tCtrl+M", ContextShowMInfo
        Menu, ContextMenuLV1, Show, % A_GuiX, % A_GuiY
    }
    Gui, ListView, MyLV2
    if (A_GuiControl = "MyLV2") && (LV_GetNext() > 0) {
        Menu, ContextMenuLV2, Add, % "Загрузить", ContextLoad
...
MainDropFiles:
    ...
    loop, parse, A_GuiEvent, `n
    {
        ...
        if (ext != "dll") {
            MsgBox, 0x1030, % "Загрузить Файл", % "Выбранный файл не является полноценной DLL. Пожалуйста, выберите валидный файл DLL или попробуйте ещё раз."
            break
        }
...
MenuCollapse:
    if !(IsExpandView) {
        Menu, ViewMenu, Check, % "Развернуть"
        GuiControl, Show, MyLV2
        Gui, Main: Show, AutoSize
        IsExpandView := true
    } else {
        Menu, ViewMenu, UnCheck, % "Развернуть"
...
LV_LoadFiles(DllFiles*)
{
    ...
    SetWindowText(hCounter, "Функции Загружаются...")
...
LV_ShowTable(Table, SetTitle)
{
    ...
    if (IsObject(Table)) {
        ...
        SetWindowText(hCounter, LV_GetCount() " Функции")
...
LV_SearchTable()
{
    ...
    if !(IsSearchPause) {
        ...
        SetWindowText(hCounter, LV_GetCount() " Функции"), SetFocus(hMyEdit1)
        GuiControl, +Redraw, MyLV1
    }
    IsSearchPause := false
}
